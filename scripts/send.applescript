...
on run argv
  ...
  tell application "System Events"
    ...
    tell appProcess
      ...
      set targetElement to value of attribute "AXFocusedUIElement"
      set targetRole to role of targetElement
      if targetRole is not "AXTextArea" and targetRole is not "AXTextField" then
        error "Codex did not focus a text editor (focused role: " & targetRole & "). Run codex-steer debug-ui <thread-id>." number 69
      end if
    end tell
  end tell
