...
`, { mode: 0o755 });
  ...
  assert.equal(customAgent.prompt, customPrompt.prompt, "Both npx forms must compose the same custom policy, with Claude as the default agent");
  assert.ok(customPrompt.prompt.endsWith(`\n\n■ 監督方針\n${policy}`), "Custom text must be preserved as literal policy text");
  assert.equal(customPrompt.prompt.split("\n\n■ 監督方針\n")[0], copiedPrompt.split("\n\n■ 監督方針\n")[0], "A custom policy must retain the mandatory template");
  ...
  assert.equal(localCustomPrompt.prompt, customPrompt.prompt, "Custom policies must also agree across local and npx installations");
