...
This checkout prepares 0.15.1, which fixes send preflight for large paginated histories and reports oversized responses as PAYLOAD_TOO_LARGE while retaining the 64 MiB receive limit. Published 0.15.0 already includes custom MESSAGE policies, supervisor controls and IDs, findings, Codex CLI launching, and `--connection desktop`. Before 0.15.1 publication, use a source installation of this branch for the fix. Existing saved-CLI deployment and v1 runtime contracts remain compatible: Desktop started with 0.15.0 does not need a restart. Existing supervisors keep their saved CLI; restart the supervisor or regenerate its prompt from the updated CLI to use the fix.
