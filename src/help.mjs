...
export const topics = {
  supervise: {
    ...
    usage: ["supervise <THREAD> [MESSAGE] [--agent claude] [-- <AGENT-ARGS...>]"],
    json_output: false,
    returns: "Claudeの標準入力・標準出力・標準エラーと終了コードを引き継ぎます。codexteerの出力は混ぜません。",
    ...
    notes: [...supervisionExecutionNotes,
      "--agentを省略するとclaudeを使い、現在の対応値もclaudeだけです。PATH上の実行ファイルを、現在の作業ディレクトリと環境変数を引き継いで起動します。シェルのaliasやfunctionは使いません。",
      "最初の -- より後ろは全てClaudeの引数です。MESSAGEはその前に置いてください。順序・空文字・空白・改行を保ち、--help/--version/--jsonもcodexteer側では解釈しません。シェルで再展開しないため、呼び出し元のシェルで必要な引用をしてください。",
      "渡された追加引数の後ろにClaude側の -- と対象ID入りの監督プロンプトを一つの引数として付加します。追加引数の意味・組み合わせの妥当性はClaudeが判断します。",
      "不正なID形式、空のMESSAGE、--agentの値不足・未対応値、区切り前の未知・過剰な引数では起動しません。Claudeが未導入・実行不可などの起動失敗は理由を標準エラーへ出して終了コード1です。Desktopの接続と対象タスクの存在は、起動した監督役が最初に確認します。",
      "SIGINT/SIGTERM/SIGHUPは起動したClaudeへ転送します。シグナルで終了した場合の終了コードは128+シグナル番号です。",
  ...
  "supervise prompt": {
    ...
    usage: ["supervise prompt <THREAD> [MESSAGE]"],
  ...
  monitor: {
    ...
    examples: ["codexteer watch <THREAD> --stream --since <CURSOR> --include-output --json", "codexteer read <THREAD> --since <LAST-CURSOR> --include-output --json", "codexteer send <THREAD> '失敗箇所を先に確認してください' --source claude-code --kind review --based-on <CURSOR> --json", "codexteer history check <THREAD> <MESSAGE-ID> --json"],
  ...
  overview: {
    ...
    notes: ["THREADにはUUIDまたはcodex://threads/...を指定します。", "監督役の対話起動: codexteer supervise <THREAD> [MESSAGE] [--agent claude] [-- <AGENT-ARGS...>]。superviseの標準入出力と終了コードはClaudeのものです。", "コマンド別のhelpには、使い所・出力・例・制約を載せています。help自体も --json に対応します。"],
  },
  read: {
