...
`codex://threads/...` のフルURLも受け付けます。待機中は現在のアプリを前面に保ち、送信キーを入力する瞬間だけCodexデスクトップへ切り替えた後、元のアプリへフォーカスを戻します。
...
このCLIは公開された [`codex://threads/<thread-id>`](https://learn.chatgpt.com/docs/reference/commands#chats) の `prompt` パラメータで対象タスクを開きます。Codex本体がメッセージをメインチャットのコンポーザーへ設定してフォーカスするため、ターミナルやファイル編集欄へ誤って貼り付けません。macOS Accessibilityに入力欄が公開されていなくても動作します。`~/.codex/config.toml` の追加入力モードも読み取り、通常送信がキューになる設定では、その1通だけをステアリングにするCodex標準ショートカットを自動で使います。ユーザー設定は変更しません。Codex内部の認証済みMCPパイプには接続しません。

制約:

- macOSと `/Applications/ChatGPT.app` が必要です。
- 実行元のTerminal、iTerm2などにAccessibility許可が必要です。
- 送信キーの入力時だけCodexデスクトップが一時的に前面へ移動します。
- 現在のUIバックエンドでは完全な無フォーカス送信ではありません。切り替えは最小化していますが、送信の瞬間に短いフォーカス移動が発生します。
...
```bash
codex-steer send 01a04373-3770-71e0-a2e3-a3c196f5f5b1 \
  "テストを先に確認して" --keep-focus
```

停止中のタスクへ通常の新規メッセージとして送信:
...
`--new-turn` を付けない場合は、実行中ターンへのステアリング用ショートカットを使います。
