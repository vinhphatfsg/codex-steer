...
| CP6: 復旧と標準化 | NOT RUN | 通常起動への復旧と再検証後に標準化する |
...
8. 全項目の証拠をこの記録に追記した後、`src/cli.mjs`の`DEFAULT_SEND_BACKEND`を`app-server`に変更する。既定方式のテスト、README、ヘルプ、skillを正式運用の記述に更新し、自動テストを再実行する。

失敗時は標準化を止め、通常起動へ戻す。起動設定はプロセス限定なので、アプリ本体の復元・設定削除・launchd解除は不要。
